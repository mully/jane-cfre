export default {
  async fetch(request, env) {
    ...
    if (url.pathname === '/health') {
      return json({ status: 'ok', service: 'jane-cfre-worker' });
    }
    ...
    return new Response('Jane CFRE Worker OK. Use /widget.js, /api/chat, or /health.', {
...
async function proxyChat(request, env) {
  const backend = env.JANE_BACKEND_URL;
  if (!backend || backend === 'REPLACE_WITH_PUBLIC_JANE_BACKEND_URL') {
    return json({ error: 'JANE_BACKEND_URL is not configured' }, 500);
  }
  ...
  return new Response(text, {
    ...
    headers: { 'Content-Type': upstream.headers.get('Content-Type') || 'application/json; charset=utf-8', ...corsHeaders() },
  });
}
...
function widgetJs() {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      wait.textContent = data.reply || data.error || 'Sorry, I had trouble answering that.';
    } catch (err) {
